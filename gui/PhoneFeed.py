...
class PhoneFeed(Tk.Frame):
    ...
    def __init__(self, parent, title=None, video=''):
        ...
        self.playButton = ttk.Button(buttons, text="Connect", command=self.OnPlay)
        ...
        self.parent.minsize(width=300, height=500)
        """
        if _isMacOS:
    ...
    while len(sys.argv) > 1:
        ...
        if arg.lower() in ('-v', '--version'):
            ...
            if not isfile(_video):
                ...
                sys.exit(1)
    _video = expanduser('../videos/dog.mp4')

    # Create a Tk.App() to handle the windowing event loop
    root = Tk.Tk()
    player = PhoneFeed(root, video=_video)
