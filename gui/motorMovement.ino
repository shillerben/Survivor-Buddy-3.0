...
void portrait(){ //phoneMountServo moves phone to portrate position
...
void clockWisePM(){ //phonemountServo moves clockwise
...
void counterClockWisePM(){ //phonemountServo moves counterclockwise
  int currPos = getPositionPM();
      int newPos = currPos +5;
      if (newPos <=180){
        phoneMountServo.write(newPos);
...
/*******************************************************************/
/*Turn Table Motor Functions*/ //360 Continuous Motor
void clockWiseTT(){ //turn cw (right)
  turnTableServo.write(0);
  //delay(1000);
}

void counterClockWiseTT(){ //turn ccw (left)
  turnTableServo.write(99);
  //dealy(1000);
}

void stopTT(){ //stop turntable movement
    turnTableServo.write(92);
    //delay(1000);
}
...
void homeServos(){
  ...
  leftBaseServo.write(0);
  rightBaseServo.write(0);
  turnTableServo.write(92); //stops motion
  phoneMountServo.write(90);
  delay(1000); 
}
/*Emergency Shut Down*/
void emergencyShutDown(){
  ...
  turnTableServo.write(92); //stops turntables 

}
/*Regular open*/
void openCompletely(){
//open arm to upright pos
  open(); //base motors open to 90 pos
}
/*Normal Shut Down*/
void shutDown(){
  //need to make sure all other motors are in correct postion to close
//close arm all the way
  portrait(); //can only close when phone is in portrait mode
  //turntable need to be in home pos (TODO)
  close(); //base motors close to 0 pos
...
void loop() { 
  ...
  if (Serial.available() > 0) {//serial is reading stuff 
    ...
    //base motor serials ****************************************
    else if (serialData == '5'){ //move left, open bm
      open();
    }
    else if(serialData == '6'){ //move right, close bm
      close(); 
    }
